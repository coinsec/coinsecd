...
func TestVirtualSelectedParentBlueScoreAndVirtualDAAScore(t *testing.T) {
	...
	response, err := coinsecd.rpcClient.GetVirtualSelectedParentBlueScore()
	if err != nil {
	...
	err = coinsecd.rpcClient.RegisterForVirtualSelectedParentBlueScoreChangedNotifications(
	...
	err = coinsecd.rpcClient.RegisterForVirtualDaaScoreChangedNotifications(
