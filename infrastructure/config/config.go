...
const (
	defaultConfigFilename      = "coinsecd.conf"
	...
	sampleConfigFilename    = "sample-coinsecd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for coinsecd.
	DefaultAppDir = util.AppDir("coinsecd", false)
...
// Flags defines the configuration options for coinsecd.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in SEC/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for coinsecd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default coinsecd configuration
...
//
// The above results in coinsecd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-coinsecd.conf to the given destination path,
