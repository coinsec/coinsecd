...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		coinsecdMessage, err := client.Post(&protowire.CoinsecdMessage{Payload: &protowire.CoinsecdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := coinsecdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	coinsecdMessage := &protowire.CoinsecdMessage{}
	err := protojson.Unmarshal([]byte(response), coinsecdMessage)
	if err != nil {
	...
	return marshalOptions.Format(coinsecdMessage)
}
