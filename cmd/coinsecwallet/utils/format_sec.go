...
// FormatSec takes the amount of sompis as uint64, and returns amount of SEC with 8 decimal places
...
func SecToSompi(amount string) (uint64, error) {
	err := validateSECAmountFormat(amount)
	...
	if constants.SompiPerCoinsec%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerCoinsec is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerCoinsec))
	decimalStr := ""
