...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("coinsecd-datadir")
	if err != nil {
	...
	spawn("startNode-coinsecdCmd.Wait", func() {
		err := coinsecdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("coinsecdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes coinsecd closes ungracefully
				log.Errorf("coinsecdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(coinsecdCmd, "coinsecdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	coinsecMinerCmd, err := common.StartCmd("MINER",
		"coinsecminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("coinsec-miner-Cmd.Wait", func() {
		err := coinsecMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(coinsecMinerCmd, "coinsecMinerCmd")
	return nil
}
